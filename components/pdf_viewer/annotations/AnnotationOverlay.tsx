...
import { BoundingBox as AllenBoundingBox, computeBoundingBoxStyle, Overlay as AllenOverlay } from '@allenai/pdf-components';
...
export function AnnotationOverlay({ 
  ...
  useEffect(() => {
    if (selectedAnnotation) {
      setActiveAnnotationId(selectedAnnotation);
    }
  }, [selectedAnnotation]);
  ...
  const shouldShowAnnotation = (annotation: Annotation) => {
    // Filter for auto-generated highlights
    if (annotation.isAutoGenerated && !filters.showAutoHighlights) {
      return false;
    }
    
    // Filter by category if categories are specified
    if (filters.categories.length > 0) {
      const categoryToCheck = annotation.category || annotation.type;
      if (!filters.categories.includes(categoryToCheck)) {
        return false;
      }
    }
